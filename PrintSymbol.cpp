...
void PrintSymbol(IDiaSymbol* pSymbol, DWORD dwIndent)
{
	...
	case SymTagBlock:
		...
		wprintf(L"%-15s: ", L"Signature");
		PrintFunctionDeclaration(pSymbol);
...
{
	PrintType(pSymbol, TRUE);
}

void PrintType(IDiaSymbol* pSymbol, BOOL bPrintUdtKind)
{
	...
	case SymTagPointerType:
		...
		PrintType(pBaseType, bPrintUdtKind);
		pBaseType->Release();
		...
		if (pSymbol->get_types(0, &count, NULL) == S_OK) {
			...
			if (pSymbol->get_types(count, &count, rgpDiaSymbols) == S_OK) {
				for (ULONG i = 0; i < count; i++) {
					PrintType(rgpDiaSymbols[i], bPrintUdtKind);
...
void PrintParameterData(IDiaSymbol* pSymbol)
{
	// Print the Kind
	/*DWORD dwDataKind;
	...
	// Print the arg's type
	IDiaSymbol* pType;

	if (pSymbol->get_type(&pType) == S_OK) {
		PrintType(pType, FALSE);
		pType->Release();
	}

	// Print the arg's name
	wprintf(L" ");
	PrintName(pSymbol);
}
...
//
void PrintFunctionDeclaration(IDiaSymbol* pSymbol)
{
	DWORD dwAccess = 0;

	DWORD dwCall;

	HRESULT hr = pSymbol->get_callingConvention(&dwCall);
	if (hr == S_OK) {
		wprintf(L"%s ", SafeDRef(rgCallingConvention, dwCall));
	}
	...
	// Get the return value and function name
	IDiaSymbol* pFuncType;
 	if (pSymbol->get_type(&pFuncType) == S_OK) {
		IDiaSymbol* pReturnType;

		if (pFuncType->get_type(&pReturnType) == S_OK) {
			PrintType(pReturnType, FALSE);
			putwchar(L' ');
	...
	if (SUCCEEDED(pSymbol->findChildren(SymTagNull, NULL, nsNone, &pEnumChildren))) {
		...
		while (SUCCEEDED(pEnumChildren->Next(1, &pChild, &celt)) && (celt == 1)) {
			if (pChild->get_symTag(&dwTag) != S_OK) {
				pChild->Release();
				wprintf(L"ERROR - can't retrieve the symbol's SymTag\n");
				continue;
			}
			...
			if (dwTag == SymTagData)
			{
				...
				PrintParameterData(pChild);
			}

			//PrintSymbol(pChild, 10);
			/*IDiaSymbol* pType;
			...
			HRESULT hr = pChild->get_type(&pType);
			if (hr == S_OK) {
				if (nParam++) {
					wprintf(L", ");
				}
